...
async function executeAgentRound(
  ...
  try {
    ...
    const completeAgentImageTask = async (image: AgentApiResultImage, rawResponsePayload?: string) => {
      ...
      // 上报生图历史到宿主后端（fire-and-forget；失败不影响 UI）
      const _cid = new URLSearchParams(window.location.search).get('cid') ?? ''
      const _hostToken = activeProfile?.apiKey || imageProfile?.apiKey || ''
      const _dataUrl = image.dataUrl || ''
      const _b64 = _dataUrl.includes(',') ? _dataUrl.split(',').slice(1).join(',') : _dataUrl
      if (_b64) {
        fetch(`${window.location.origin}/api/v1/image-gen/history/callback`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${_hostToken}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({
            customer_id: _cid ? Number(_cid) : null,
            prompt: image.revisedPrompt ?? latestBeforeUpdate.prompt ?? '',
            model: activeProfile?.model || imageProfile?.model || '',
            size: actualParams?.size ?? null,
            quality: actualParams?.quality ?? null,
            status: 'success',
            image_b64: _b64,
            latency_ms: 0,
          }),
    ...
    while (true) {
      ...
      const customerProfileFunctionCalls = currentResponseOutputItems.filter(
        (item) => item.type === 'function_call' && item.name === 'get_customer_profile',
      )
      ...
      // get_customer_profile：拉取宿主后端的脱敏客户摘要，回填给模型（标准 function_call_output 闭环）
      if (customerProfileFunctionCalls.length > 0) {
        const cid = new URLSearchParams(window.location.search).get('cid') ?? ''
        const hostToken = activeProfile?.apiKey || imageProfile?.apiKey || ''
        for (const fc of customerProfileFunctionCalls) {
          let output: string
          if (!cid) {
            output = JSON.stringify({ error: 'no customer context (cid missing in host)' })
          } else {
            try {
              const r = await fetch(`${window.location.origin}/api/v1/image-gen/customer-profile/${encodeURIComponent(cid)}`, {
                headers: { Authorization: `Bearer ${hostToken}` },
              })
              const data = r.ok ? await r.json().catch(() => ({} as Record<string, unknown>)) : {}
              output = (data && typeof data.context_text === 'string' && data.context_text)
                || JSON.stringify(data && Object.keys(data).length ? data : { error: `profile HTTP ${r.status}` })
            } catch (err) {
              output = JSON.stringify({ error: `profile fetch failed: ${err instanceof Error ? err.message : String(err)}` })
            }
          }
          functionCallOutputs.push({ type: 'function_call_output', call_id: fc.call_id, output })
        }
      }
