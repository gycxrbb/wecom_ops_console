...
function createAgentInstructions(settings: AppSettings) {
  ...
  const instructions = [
    imageInstructions,
    '',
    '## 客户画像确认（get_customer_profile）',
    '- 调用 get_customer_profile 后，必须先用 2-4 句中文向教练概括你拿到的客户关键信息：健康状况/慢性病、体型（BMI/体脂）、饮食与运动偏好、当前主要目标。',
    '- 这一步是让教练核对你理解是否正确；概括要简洁、口语化（别罗列原始字段名），确认后再开始生成图片。',
    '- 不要在回复里输出形如 <..._DONE> 的控制/停止标记（那是系统用的，不能让用户看到）。',
    '',
    '## Tool policy',
...
function createAgentTools(params: TaskParams, profile: ApiProfile, settings: AppSettings, maskDataUrl?: string): Array<Record<string, unknown>> {
  ...
  // get_customer_profile: 宿主嵌入模式下，拉取当前客户的脱敏健康/生活摘要（无 PII）。
  // 无参 —— 客户由宿主 iframe 的 cid 决定。模型据此生成更贴合客户的图（食谱卡/问候/营养科普等）。
  tools.push({
    type: 'function',
    name: 'get_customer_profile',
    description: [
      'Fetch the current customer\'s desensitized health & lifestyle profile (no PII) from the host system.',
      'Call this FIRST when the user wants an image tailored to a specific customer — e.g. a recipe card, daily greeting, or nutrition tip based on their situation.',
      'Use the returned summary (health conditions, body composition, goals, diet/exercise preferences) to write a more relevant, personalized image prompt.',
      'Takes no arguments — the customer is determined by the host context automatically.',
    ].join(' '),
    parameters: {
      type: 'object',
      properties: {},
...
export async function callAgentResponsesApi(opts: {
  ...
  try {
    const body: Record<string, unknown> = {
      // 宿主嵌入模式：agent /responses 用推理模型（URL param agentModel 覆盖），
      // 与画廊生图模型(model=gpt-image-2)区分——agent 需要能调 tool 的推理模型如 gpt-4o-mini。
      model: (typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('agentModel')) || profile.model || settings.model,
